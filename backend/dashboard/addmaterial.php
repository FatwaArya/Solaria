...
//                $result = move_uploaded_file($file['tmp_name'], $uploadPath);
    ...
    <div class="log">
        ...
        <table class="pure-table ">
            ...
            $no = 0;
            while ($solaria_data = mysqli_fetch_array($result)) {
                $no++; ?>
                <tr>

                    <td><?= $no ?></td>
                    <td><?= $solaria_data['nama_materi'] ?> </td>
                    <td><?= $solaria_data['date'] ?></td>
                    <td><?= $solaria_data['description'] ?></td>



                    <td class="action" name="action">
                        <a href="user_update.php? id= <?php echo $solaria_data['id'] ?>"
                           class="button-small pure-button pure-button-primary">CHG</a>

                        <a href="del_data.php?    id= <?php echo $solaria_data['id'] ?> "
                           class=" pure-button pure-button">DEL</a>
